...
List * createList() {
    List *list = (List*)malloc(sizeof(List)); //reservo memoria 
    if (list == NULL) return NULL; // pregunto si hay datos dentro de list
    list->head = NULL; // head lo inicio en NULL
    list->tail = NULL; // tail lo inicio en NULL
    list->current = NULL; //  current o inicio en NULL
...
void pushFront(List * list, void * data) {
    if (list == NULL ) return; // verifico si la lista exista
    Node *new = createNode(data); // creo un nuevo nodo con el dato ya dado
    if (list->head == NULL) { // veo si en la lista hay elementos
        list->head = new; // new ahora es el primero
        list->tail = new; // new tambien es el ultimo
        list->current = new; //new es el actual
        return;
    }
    new->next = list->head; // new lo conecto al antiguo primer nodo
    list->head->prev = new; // ahora el prev lo actualizo para que apunte al new
    list->head = new; // actualizo el head para que apunte al new
...
void * popCurrent(List * list) {
    ...
    // verifico si existe la list y haya un nodo actual
    Node *izq = list->current->prev; // guardo el nodo anterior en izq
    Node *der = list->current->next; // guardo el nodo siguente en der
    void *data = list->current->data; // guardo el dato antes de eliminarlo
    Node *eliminado = list->current; // guardo el nodo eliminado

    if (izq != NULL) izq->next = der; 
    // si es que existe un nodo anterior, next apunta a la derecha
    else list->head = der; // si no existe, entonces actualizo el head

    if (der != NULL) der->prev = izq; 
    // si existe nodo siguente, prev ahora punta al izquierdo
    else list->tail = izq; // si no existe, actualizo el tail

    list->current = der; //ahora current queda apuntado a la derecha del eliminado
    free(eliminado); // libero memoria del eliminadp

    return data; // retorno el dato que estaba almacenado en el eliminado
}
